...
class Test_EndtoEnd_OneArmCont_SingleMean_Ad_Results:

    baseURL = ReadConfig.getApplicationURL()
    username = ReadConfig.getUserName()
    password = ReadConfig.getPassword()
    wrapperTestData = ReadConfig.getwrapperTD("E2E_OneArm_ADResults_SingleMean")
    ...
    def test_EndtoEnd_OneArmCont_SingleMean_Ad_Results(self,testcase,setup,extra,request):
        LogGen.loggen().info(f"One-Arm-Continuous -SingleMean- {testcase} started")
            ...
            for row_index, row_data in datafile.iterrows():
                try:                    
                    ...
                    try:
                        ...
                        self.DesignHelper.process_input_row_SingleMean(row_data)    
                    ...
                    try:
                        ...
                        self.DesignHelper.validate_results(datafile, row_data, LogScreenshot, tc_status)
                    except Exception as e:
                        LogScreenshot.fLogScreenshot( message = f"Error in Generating results : {e}",
                            pass_ = False, log = True, screenshot = True) 
                        tc_status.append("FAIL") 
        ...
        self.driver.quit()
        LogGen.loggen().info(f"One-Arm-Continuous Results- {testcase} completed")
