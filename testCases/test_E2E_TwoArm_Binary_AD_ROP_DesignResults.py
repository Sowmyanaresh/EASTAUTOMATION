...
class Test_EndtoEnd_TwoArm_Binary_ROP_ComputeResults:

    baseURL = ReadConfig.getApplicationURL()
    username = ReadConfig.getUserName()
    password = ReadConfig.getPassword()
    wrapperTestData = ReadConfig.getwrapperTD("E2E_TwoArm_ADResults_ROP")
    ...
    def test_EndtoEnd_TwoArm_Binary_ROP_ComputeResults(self,testcase,setup,extra,request):
        LogGen.loggen().info(f"Two-Arm-Binary -ROP- {testcase} started")
        ...
        '''
        datafile = pd.read_excel( path, sheet_name = "TestdataNew" )
        ...
        for tc_index in range(len(self.TCList)):
            ...
            try:
                ...
                '''
                LogScreenshot.fLogScreenshot( message = f"ROP Design workflow started", pass_ = True, log = True, screenshot = True)
            ...
            for row_index, row_data in datafile.iterrows():
                try:                    
                    ...
                    try:
                        ...
                        self.DesignHelper.process_input_row_DOP_ROP(row_data)
                    ...
                    try:            
                        ...
                        time.sleep(5)
                        LogScreenshot.fLogScreenshot( message = f"Clicked on Results-{row_index + 1} inputset Page ",
                            pass_ = True, log = True, screenshot = True)
                    except Exception as e:
                    ...
                    try:
                        ...
                        self.DesignHelper.validate_results_DOP(datafile, row_data, LogScreenshot, tc_status)
                    except Exception as e:
                        LogScreenshot.fLogScreenshot( message = f"Error in Generating results : {e}",
                            pass_ = False, log = True, screenshot = True) 
                        tc_status.append("FAIL") 
        ...
        self.driver.quit()
        LogGen.loggen().info(f"Two-Arm-Binary Results- {testcase} completed")
