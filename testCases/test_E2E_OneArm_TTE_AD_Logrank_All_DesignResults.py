...
class Test_EndtoEnd_OneArmCont_Logrank_ComputeResults:

    baseURL = ReadConfig.getApplicationURL()
    username = ReadConfig.getUserName()
    password = ReadConfig.getPassword()
    wrapperTestData = ReadConfig.getwrapperTD("E2E_SingleArm_ADResults_TTE")
    ...
    def test_EndtoEnd_OneArm_TTE_Logrank_ComputeResults(self,testcase,setup,extra,request):
        LogGen.loggen().info(f"One-Arm-TTE - {testcase} started")
        ...
        '''
        #datafile = pd.read_excel( path, sheet_name = "Testdata-LogrankWeibull", keep_default_na=False)
        datafile= pd.read_excel( path, sheet_name = "TestData-Exponential" )
        #datafile= pd.read_excel( path, sheet_name = "Testdata-ParametricWeibull" )
            ...
            for row_index, row_data in datafile.iterrows():
                try:                    
                    ...
                    try:
                        ...
                        self.DesignHelper.process_input_row_SingleArmTTE(row_data)    
                    ...
                    try:
                        ...
                        self.DesignHelper.validate_results_SingleSurvival(datafile, row_data, LogScreenshot, tc_status)
                    except Exception as e:
                        LogScreenshot.fLogScreenshot( message = f"Error in Generating results : {e}",
                            pass_ = False, log = True, screenshot = True) 
                        tc_status.append("FAIL") 
        ...
        self.driver.quit()
        LogGen.loggen().info(f"Single-Arm-Continuous Results- {testcase} completed")
